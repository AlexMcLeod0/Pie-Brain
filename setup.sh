...
if [[ -d "$INSTALL_DIR" ]]; then
    ...
    [[ "${_ow,,}" == "y" ]] || die "Aborting. Choose a different directory or remove it manually."
    rm -rf "$INSTALL_DIR"
fi
...
declare -A _keep_tools=()
for _t in "${TOOLS[@]}"; do _keep_tools[$_t]=1; done

for _pair in "arxiv:arxiv" "memory:memory" "git_sync:git_sync"; do
    _tool="${_pair%%:*}"
    _stem="${_pair##*:}"
    if [[ -z "${_keep_tools[$_tool]+_}" ]]; then
